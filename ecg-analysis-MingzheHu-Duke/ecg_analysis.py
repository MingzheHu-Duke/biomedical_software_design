def user_input():
    ...
    This function lists all the files in the current folder and ask users to
    ...
    """
    import os
    print("Your files in the current folder is:")
    # List all the files in the current path
    print(os.listdir("."))
    file_name = input("Please enter the file name you want to analysis:")
    return file_name
...
def data_check(file_name):
    ...
    Check if missing data exists in the .csv file. If exists raise an Exception
    ...
    for i in range(len(data[:, 0])):
        if math.isnan(data[i, 1]) is True or math.isnan(data[i, 0]) is True:
...
def nan_interpolate(data):
    ...
    If NANs exist. If it is the first element, replace it with the value behind
...
def ecg_calculator(file_name, processed_data, peaks):
    ...
    Calculate the metrics of ecg and generate a dict to store the information
    in the keys
    ...
    if min <= -300 or max >= 300:
        ecg_logging(1, "Abnormal voltages!"
                       " file-{} max-{}, min-{}".format(file_name, max, min))
    # Include minimum and maximum lead values
    voltage_extremes = (min, max)
    ecg_logging(0, "Calculating voltage_extremes :{}".format(voltage_extremes))
...
def user_output(metrics):
    ...
    Print out all the metrics values and let users to decide if they need to
    plot the ECG
    ...
    flag = input("Do you want to plot the ECG signal (y/n)?")
    return flag
...
def main():
    ...
    flag = user_output(metrics)
    if flag == "y":
        ecg_logging(0, "Plotting enabled")
        plot_ecg(processed_data, clean_data, peaks)
